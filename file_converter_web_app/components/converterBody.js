...
export default function ConverterBody({ onConvert, isImageConverter }) {
    ...
    function handleInputDataChange(data) {
        ...
        if (onConvert) {
            try {
                convertedData = onConvert(convertedData);
            } catch (error) {
                convertedData = `Cannot convert this string:\n ${data}`
            }
        ...
        <div className="w-full h-screen flex flex-row justify-center mt-5 gap-10 pt-12">
            ...
            <div className="w-1/3 h-96 relative">
                <textarea
                    ...
                    className="w-full h-full rounded-lg block bg-sky-100 p-1 max-w-full"
                    placeholder="Result"
                    value={!isImageConverter ? outputData : ''}
                />
                {isImageConverter && (
                    <img src={outputData} alt="converted-img.png" className="absolute top-0 left-0 w-full h-full border border-black bg-white text-xs text-start" />
                )}
                {!isImageConverter ? (
                    <button
                        className="absolute bottom-5 right-5 bg-slate-400 hover:bg-slate-500 active:ring active:bg-slate-600 rounded-full w-12 h-12 flex items-center justify-center"
